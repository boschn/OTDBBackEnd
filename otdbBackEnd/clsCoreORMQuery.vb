...
#Region "Property"
        ...
        Public Property ObjectEntryNames As IList(Of String) Implements iormQueriedEnumeration.ObjectEntryNames
            ...
            Set(value As IList(Of String))
                ...
                For Each aName In value
                    If CurrentSession.Objects.GetObject(objectid:=_objectid).HasEntry(entryname:=aName) Then
                        If CurrentSession.Objects.GetObject(objectid:=_objectid).GetEntry(entryname:=aName).IsColumn Then
        ...
        Public ReadOnly Property ObjectDefinition As ObjectDefinition
            Get
                Return CurrentSession.Objects.GetObject(objectid:=_objectid)
            End Get
        End Property
        ''' <summary>
        ...
        Public ReadOnly Property ObjectHasDomainBehavior As Boolean
            Get
                Dim aDescription = ot.GetObjectClassDescriptionByID(id:=_objectid)
                If aDescription Is Nothing Then Return False

                '** avoid loops during startup and domain switching
                If (CurrentSession.IsStartingUp OrElse CurrentSession.IsDomainSwitching) AndAlso _
                    ot.GetBootStrapObjectClassIDs.Contains(aDescription.ID) Then
                    Return aDescription.ObjectAttribute.AddDomainBehavior
        ...
        Public ReadOnly Property ObjectHasDeleteBehavior As Boolean
            Get
                ...
                If (CurrentSession.IsStartingUp OrElse CurrentSession.IsDomainSwitching) AndAlso _
                    ot.GetBootStrapObjectClassIDs.Contains(aDescription.ID) Then
                    Return aDescription.ObjectAttribute.AddDeleteFieldBehavior
...
#End Region
        ...
        Public Sub New(Optional id As String = Nothing)
            ...
            Else
                ...
                Dim aList As New SortedList(Of UInt16, String)
                Dim i As Integer = 1
                For Each anEntry In CurrentSession.Objects.GetObject(objectid:=_objectid).PrimaryKeyEntryNames
                    aList.Add(i, anEntry)
                    i += 1
                Next
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function GetObject(no As ULong) As iormRelationalPersistable Implements iormQueriedEnumeration.GetObject
            If Not Me.IsLoaded AndAlso Not Me.Load() Then
        ...
        Public Function GetObjectDefinition() As ObjectDefinition Implements iormQueriedEnumeration.GetObjectDefinition
            Return ot.CurrentSession.Objects.GetObject(objectid:=_objectid)
        End Function
        ''' <summary>
        ...
        Public Function GetObjectEntries() As IList(Of iormObjectEntry) Implements iormQueriedEnumeration.GetObjectEntries
            Dim newList As New List(Of iormObjectEntry)
            For Each anEntryname In _entrynames
                If CurrentSession.Objects.GetObject(objectid:=_objectid).HasEntry(anEntryname.Value) Then
                    Dim anObjectEntry As iormObjectEntry = CurrentSession.Objects.GetObject(objectid:=_objectid).GetEntry(anEntryname.Value)
        ...
        Public Function GetObjectEntry(name As String) As iormObjectEntry Implements iormQueriedEnumeration.GetObjectEntry
            If _entrynames.Values.Contains(name.ToUpper) Then
                Return CurrentSession.Objects.GetObject(objectid:=_objectid).GetEntry(entryname:=name)
            Else
                Return Nothing
            End If
        End Function
        ...
        Private Function BuildSelectCommand(Optional domainid As String = Nothing) As ormSqlSelectCommand
            ...
            Try
                ...
                '** get a Store
                Dim aStore As iormRelationalTableStore = CurrentSession.CurrentDBDriver.GetTableStore(primaryTableID)
                ...
                If Not aSelectCommand.IsPrepared Then
                    ...
                    For i = 0 To aDescription.PrimaryKeyEntryNames.Count - 1
                        [select] &= "[" & CurrentDBDriver.GetNativeDBObjectName(primaryTableID) & "].[" & aDescription.PrimaryKeyEntryNames(i).ToUpper & "]"
                        If i > 0 Then [select] &= " , "
                    Next
                    ...
                    For Each aName In _entrynames.Values
                        If Not aDescription.PrimaryKeyEntryNames.Contains(aName) Then
                            If aName.Contains(".") Then
                                [select] &= ",[ " & aName & "]"
                            Else
                                [select] &= ", [" & CurrentDBDriver.GetNativeDBObjectName(primaryTableID) & "].[" & aName & "]"
                            End If
                        End If
                    Next
                    ...
                    If Me.ObjectHasDomainBehavior Then
                        ...
                        If Not aDescription.PrimaryKeyEntryNames.Contains(Domain.ConstFNDomainID.ToUpper) AndAlso _
                            ...
                            [select] &= ", [" & CurrentDBDriver.GetNativeDBObjectName(primaryTableID) & "].[" & Domain.ConstFNDomainID.ToUpper & "]"
                        End If
                    ...
                    If Me.ObjectHasDeleteBehavior Then
                        ...
                        If Not aDescription.PrimaryKeyEntryNames.Contains(ormBusinessObject.ConstFNIsDeleted.ToUpper) AndAlso _
                          Not _entrynames.Values.Contains(ormBusinessObject.ConstFNIsDeleted.ToUpper) Then
                            [select] &= ", [" & CurrentDBDriver.GetNativeDBObjectName(primaryTableID) & "].[" & ormBusinessObject.ConstFNIsDeleted.ToUpper & "]"
                        End If
        ...
        Private Function BuildSelectCommandFromQryAttribute(Optional domainid As String = Nothing) As ormSqlSelectCommand
            ...
            Try
                ...
                Dim aStore = TryCast(CurrentDBDriver.RetrieveContainerStore(containerid:=Me.ObjectDefinition.PrimaryTableId), iormRelationalTableStore)
        ...
        Public Function GetObject(no As ULong) As iormRelationalPersistable Implements iormQueriedEnumeration.GetObject
            ...
            If _run Then
                If no < _records.Count Then
                    Dim newObject As iormRelationalPersistable = ot.CreateDataObjectInstance(_objecttype)
        ...
        Public Function Load(Optional domainid As String = Nothing) As Boolean Implements iormQueriedEnumeration.Load
            ...
            If _select.IsPrepared Then
                ...
                If hasDomainBehavior And domainid <> ConstGlobalDomain Then
                    ...
                    Dim aDomainRecordCollection As New Dictionary(Of String, ormRecord)
                    Dim pknames = CurrentSession.CurrentDBDriver.RetrieveContainerSchema(_select.TableIDs.First).PrimaryEntryNames
