...
Namespace OnTrack.Database
    ...
    Public MustInherit Class ormRelationalInfusable
        ...
        ''' <summary>
        ''' event on loading the relation from the databse
        ''' </summary>
        ''' <param name="sender"></param>
        ''' <param name="e"></param>
        ''' <remarks></remarks>
...
#Region "Properties"
        ...
        Public ReadOnly Property ObjectEntryDefaultValue(entryname As String) As Object Implements iormInfusable.ObjectEntryDefaultValue
            Get
                If Me.ObjectDefinition Is Nothing Then
                    Dim anEntryAttribute As iormObjectEntry = Me.ObjectClassDescription.GetObjectEntryAttribute(entryname)
                    If anEntryAttribute Is Nothing Then Throw New ormException(message:="entry name '" & entryname & "' in object class description '" & Me.ObjectID & "' not found", subname:="ormInfusable.ObjectEntryDefaultValue")
                ...
                Else
                    ...
                    If anEntry Is Nothing Then Throw New ormException(message:="entry name '" & entryname & "' in object '" & Me.ObjectID & "' not found", subname:="ormInfusable.ObjectEntryDefaultValue")
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Overrides ReadOnly Property ObjectPrimaryKeyValues As Object() Implements iormDataObject.ObjectPrimaryKeyValues
            Get
...
#End Region
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function GetValue(entryname As String, Optional ByRef fieldmembername As String = Nothing) As Object Implements iormInfusable.GetValue
            ...
            Try
                ...
                If Not aClassDescription.HasObjectEntryAttribute(entryname) Then
                    If CurrentSession.IsBootstrappingInstallationRequested OrElse CurrentSession.IsStartingUp Then
                        CoreMessageHandler(message:="Object entry does not exist in object class description - on bootstrap installation or session startup compounds are not allowed to be used. ", _
                                       argument:=entryname, objectname:=Me.ObjectID, entryname:=entryname, _
                                       messagetype:=otCoreMessageType.InternalError, procedure:="ormInfusable.GetValue")
                    Else
                ...
                ElseIf thefieldinfos.Count > 1 And String.IsNullOrWhiteSpace(fieldmembername) Then
                    ''' ????
                End If
                ...
                If anEntryAttribute Is Nothing Then
                    CoreMessageHandler(message:="object entry attribute couldnot be retrieved from class description", argument:=value, _
                                       objectname:=Me.ObjectID, entryname:=entryname, _
                                        messagetype:=otCoreMessageType.InternalError, procedure:="ormInfusable.GetValue")
                    Return Nothing
                End If
                ...
                For Each field In thefieldinfos
                    ...
                    If fieldmembername = String.Empty Then
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Function SetValue(entryname As String, ByVal value As Object) As Boolean Implements iormInfusable.SetValue
            Dim result As Boolean = False
            Dim isnullable As Boolean = False
        ...
        Public Function Feed(Optional record As ormRecord = Nothing) As Boolean Implements iormInfusable.Feed
            ...
            '** Fire Class Event
            Dim ourEventArgs As New ormDataObjectEventArgs(Me, record:=record, key:=Me.ObjectPrimaryKey, usecache:=Me.UseCache)
        ...
        Public Function Infuse(ByRef record As ormRecord, Optional mode? As otInfuseMode = Nothing) As Boolean Implements iormInfusable.Infuse
            ...
            Try
                Dim aPrimaryKey As ormDatabaseKey = ExtractObjectPrimaryKey(record:=record, objectID:=Me.ObjectID, runtimeOnly:=Me.RunTimeOnly)
                '** Fire Event
                Dim ourEventArgs As New ormDataObjectEventArgs(Me, record:=record, key:=aPrimaryKey, usecache:=Me.UseCache, infusemode:=mode, _
                ...
                ourEventArgs = New ormDataObjectEventArgs(Me, record:=record, key:=aPrimaryKey, infusemode:=mode, runtimeOnly:=Me.RunTimeOnly, usecache:=Me.UseCache)
        ...
        Private Function InfuseColumnMapping(mode As otInfuseMode) As Boolean
            ...
            Try
                ...
                For Each aColumnName In Me.ObjectClassDescription.MappedContainerEntryNames
                    Dim aFieldList As List(Of FieldInfo) = Me.ObjectClassDescription.GetMappedContainerEntry2FieldInfos(containerEntryName:=aColumnName)
        ...
        Protected Shared Function InfusePrimaryKeys(ByRef dataobject As iormInfusable, _
            ...
            If Not runtimeOnly Then
                Dim atablestore As iormRelationalTableStore = ot.GetTableStore(aDescriptor.Tablenames.First)
        ...
        Public Shared Function InfuseDataObject(ByRef record As ormRecord, ByRef dataobject As iormInfusable, _
            ...
            If dataobject Is Nothing Then
                CoreMessageHandler(message:="data object must not be nothing", procedure:="ormInfusable.InfuseDataObject", _
                                   messagetype:=otCoreMessageType.InternalError, _
                                    containerID:=record.TableIDs.First)
                Return False
            End If
            '** extract primary keys
            Dim aPrimaryKey As ormDatabaseKey = ExtractObjectPrimaryKey(record:=record, objectID:=dataobject.ObjectID, runtimeOnly:=dataobject.RuntimeOnly)
            '** Fire Event
            Dim ourEventArgs As New ormDataObjectEventArgs(dataobject, record:=record, key:=aPrimaryKey, usecache:=dataobject.useCache, infuseMode:=mode)
            ...
            ourEventArgs = New ormDataObjectEventArgs(dataobject, record:=record, key:=aPrimaryKey, usecache:=dataobject.useCache, infuseMode:=mode)
