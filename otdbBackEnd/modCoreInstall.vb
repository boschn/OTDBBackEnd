...
Namespace OnTrack.Database
    ...
    Public Module Installation
        ...
        Public Sub CreateDatabase(Optional modules As IEnumerable(Of String) = Nothing, Optional force As Boolean = False, Optional setupid As String = Nothing)
            ...
            End If
            '** create the db table
            result = result And CurrentDBDriver.CreateDBParameterContainer(nativeConnection:=aNativeConnection)

            '*** get the current schema version
            Dim schemaversion = CurrentDBDriver.GetDBParameter(parametername:=ConstPNBSchemaVersion, SetupID:=setupid, silent:=True)
            ...
            For Each description In descriptions
                ...
                For Each aContainerID In description.ContainerIDs
                    Dim value = CurrentDBDriver.GetDBParameter(parametername:=ConstPNBSchemaVersion_TableHeader & aContainerID, setupID:=setupid, silent:=True)
                    If value Is Nothing OrElse Not IsNumeric(value) OrElse Not CurrentDBDriver.HasContainerID(aContainerID) Then
                        addflag = True
                    ElseIf Convert.ToUInt64(value) > description.GetContainerAttribute(aContainerID).Version Then
                        CoreMessageHandler(message:="WARNING ! Version of Bootstrapping Table in database is higher ( " & value & ") than in class description ( " & description.GetContainerAttribute(aContainerID).Version & "). Downgrading ?!", messagetype:=otCoreMessageType.InternalWarning, _
                    ...
                    ElseIf force OrElse Convert.ToUInt64(value) < description.GetContainerAttribute(aContainerID).Version Then
                        addflag = True
                    End If
                Next
            ...
            '*** bootstrap checksum
            CurrentDBDriver.SetDBParameter(ConstPNBootStrapSchemaChecksum, value:=ot.GetBootStrapSchemaChecksum, SetupID:=setupid, silent:=True)
                ...
                For Each description In descriptions
                    ...
                    For Each aContainerID In description.ContainerIDs
                        ...
                        If value Is Nothing OrElse Not IsNumeric(value) OrElse Not CurrentDBDriver.HasContainerID(aContainerID) Then
                            addflag = True
                        ElseIf Convert.ToUInt64(value) > description.GetContainerAttribute(aContainerID).Version Then
                            CoreMessageHandler(message:="WARNING ! Version of Container in database is higher ( " & value & ") than in class description ( " & description.GetContainerAttribute(aContainerID).Version & "). Downgrading ?!", messagetype:=otCoreMessageType.InternalWarning, _
                                                procedure:="Installation.createDatabase", containerID:=aContainerID, objectname:=description.ID, argument:=description.GetContainerAttribute(aContainerID).Version)
                        ElseIf force OrElse Convert.ToUInt64(value) < description.GetContainerAttribute(aContainerID).Version Then
                            addflag = True
                        End If
            ...
            For Each modulename In modules
                If modulename <> ConstModuleCommons Then
                    ...
                    For Each description In descriptions
                        ...
                        For Each aContainerID In description.ContainerIDs
                            Dim value = CurrentDBDriver.GetDBParameter(ConstPNBSchemaVersion_TableHeader & aContainerID, setupID:=setupid, silent:=True)
                            If value Is Nothing OrElse Not IsNumeric(value) OrElse Not CurrentDBDriver.HasContainerID(aContainerID) Then
                                addflag = True
                            ElseIf Convert.ToUInt64(value) > description.GetContainerAttribute(aContainerID).Version Then
                                CoreMessageHandler(message:="WARNING ! Version of Table in database is higher ( " & value & ") than in class description ( " & description.GetContainerAttribute(aContainerID).Version & "). Downgrading ?!", messagetype:=otCoreMessageType.InternalWarning, _
                                                    procedure:="Installation.createDatabase", containerID:=aContainerID, objectname:=description.ID, argument:=description.GetContainerAttribute(aContainerID).Version)
                            ElseIf force OrElse Convert.ToUInt64(value) < description.GetContainerAttribute(aContainerID).Version Then
                                addflag = True
                            End If
            ...
            CurrentDBDriver.SetDBParameter(parametername:=ConstPNBSchemaInstallationDate, value:=Now.ToString, SetupID:=setupid, silent:=True)
            CurrentDBDriver.SetDBParameter(parametername:=ConstPNBackendVersion, value:=ot.AssemblyVersion.ToString, SetupID:=setupid, silent:=True)
            CurrentDBDriver.SetDBParameter(parametername:=ConstPNBSchemaVersion, value:=ot.SchemaVersion, SetupID:=setupid, silent:=True)
        ...
        Public Function DropDatabase() As Boolean
            '** check rights
            If ot.CurrentConnection.IsConnected OrElse CurrentDBDriver.HasAdminUserValidation() Then
            ...
            ''' 
            Dim droppedTables As New List(Of String)
            Dim theDependenciesList As New Dictionary(Of String, List(Of String))
            Dim allTables As List(Of ormTableAttribute) = ot.GetTableAttributes
            Dim deleted As Long = 0

            ''' check all tables -> build net
            ''' 
            For Each aTableAttribute In allTables.ToList
                Dim delete As Boolean = False

                ''' check if foreign key references 
                If Not CurrentDBDriver.HasContainerID(aTableAttribute.TableID) Then
                    '** if table doesnot exist then remove from alltables
                    allTables.Remove(aTableAttribute)
                    ''' delete also the DB Parameter -> do not go over the table object
                    ''' since we are not connected
                    CurrentDBDriver.DeleteDBParameter(parametername:=ConstPNBSchemaVersion_TableHeader & aTableAttribute.TableID.ToUpper, silent:=True)

                    ''' else if exists and has foreign keys
                ElseIf aTableAttribute.ForeignKeyAttributes.Count <> 0 Then

                    For Each aforeignkey In aTableAttribute.ForeignKeyAttributes
                            ...
                            For Each aReference In aforeignkey.ForeignKeyReferences
                                ...
                                If names.Count > 1 Then
                                    ...
                                    If anRefObjectEntry IsNot Nothing AndAlso anRefObjectEntry.HasValueContainerID _
                                        AndAlso CurrentDBDriver.HasContainerID(anRefObjectEntry.ContainerID) Then
                                        ...
                                        End If
                                        '* add this tableid to be dependend from
                                        If Not aDependendFromList.Contains(aTableAttribute.TableID.ToUpper) Then
                                            aDependendFromList.Add(aTableAttribute.TableID.ToUpper)
                                        End If
                                    End If
                                End If
            ...
            ''' 
            Dim alltablesCount As Integer = allTables.Count
            Do
                deleted = 0
                ''' check all tables
                For Each aTableAttribute In allTables.ToList
                    ''' check if foreign key references 
                    If Not theDependenciesList.ContainsKey(aTableAttribute.TableID.ToUpper) Then
                        '** Drop Table
                        If CurrentDBDriver.DropContainerObject(aTableAttribute.TableID) Then
                            CoreMessageHandler(message:="TABLE  " & aTableAttribute.TableID.ToUpper & " dropped", containerID:=aTableAttribute.TableID.ToUpper, _
                        ...
                        droppedTables.Add(aTableAttribute.TableID.ToUpper)
                        ...
                        For Each aDependList In theDependenciesList
                            If aDependList.Value.Contains(aTableAttribute.TableID.ToUpper) Then
                                aDependList.Value.Remove(aTableAttribute.TableID.ToUpper)
                            End If
                    ...
                    End If
                Next
            Loop While deleted <> 0 AndAlso allTables.Count > 0


            ''' drop the setup
            '''
            CurrentDBDriver.DropDBParameterContainer(setupid:=CurrentSetupID)
            ...
            With New UI.CoreMessageBox
                ...
                '* Message
                If droppedTables.Count = alltablesCount Then
                    .Message = droppedTables.Count & " tables of the database have been dropped." & vbLf & _
                        "Database was deleted."
                Else
                    .Message = droppedTables.Count & " tables from a total of " & alltablesCount & " in the database have been dropped." & vbLf & _
            ...
            '* return
            If droppedTables.Count = allTables.Count Then
                Return True
            Else
                Return False
            End If
        ...
        Public Function InitializeData(setupid As String, Optional searchpath As String = Nothing) As Boolean
            ...
            '***
            Dim valueFrom As Object = CurrentDBDriver.GetDBParameter(ConstPNCalendarInitializedFrom, SetupID:=setupid, silent:=True)
            Dim valueTo As Object = CurrentDBDriver.GetDBParameter(ConstPNCalendarInitializedto, SetupID:=setupid, silent:=True)
            ...
            If valueFrom Is Nothing OrElse valueTo Is Nothing _
                ...
                Else
                    ot.CoreMessageHandler(showmsgbox:=False, procedure:="Installation.createDatabase", _
                                                         ...
                                                         messagetype:=otCoreMessageType.InternalInfo)
                    CurrentDBDriver.SetDBParameter(ConstPNCalendarInitializedFrom, SetupID:=setupid, value:=Format(fromDate, "yyyy-MM-dd"))
                    CurrentDBDriver.SetDBParameter(ConstPNCalendarInitializedto, SetupID:=setupid, value:=Format(ToDate, "yyyy-MM-dd"))
                End If
            End If
            ...
            ''' 
            If Not String.IsNullOrWhiteSpace(DefaultFolder) AndAlso IO.Directory.Exists(searchpath) Then
            ...
            ElseIf IO.Directory.Exists(System.IO.Path.GetDirectoryName(uri.LocalPath) & "\Resources\" & DefaultFolder) Then
            ...
            If Not String.IsNullOrWhiteSpace(DefaultFolder) AndAlso IO.Directory.Exists(searchpath) Then
                ...
                FeedInInitialData(searchpath)
            ElseIf IO.Directory.Exists(System.IO.Path.GetDirectoryName(uri.LocalPath) & "\Resources\" & setupid) Then
