...
#End Region
    ...
    ''' <summary>
    ''' abstract ado.net database driver
    ''' </summary>
    ''' <remarks></remarks>
    Public MustInherit Class adonetDBDriver
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Overrides Function VerifyTableSchema(tabledefinition As ContainerDefinition, Optional ByRef connection As iormConnection = Nothing, Optional nativeConnection As Object = Nothing) As Boolean
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Overrides Function VerifyContainerSchema(containerAttribute As iormContainerAttribute, Optional ByRef connection As iormConnection = Nothing, Optional nativeConnection As Object = Nothing) As Boolean
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Overrides Function DropTable(ByVal id As String, Optional ByRef connection As iormConnection = Nothing) As Boolean Implements iormRelationalDatabaseDriver.DropContainerObject
            Dim nativename As String = Me.GetNativeDBObjectName(id)
            '** drop table
            Me.RunSqlStatement("DROP TABLE " & nativename)
            Return Not Me.HasTable(id)
        End Function
        ...
        ''' <param name="addToSchemaDir">The add to schema dir.</param>
        ''' <returns></returns>
        Public Overrides Function GetColumn(nativeTable As Object, columndefinition As ContainerEntryDefinition, Optional createOrAlter As Boolean = False, Optional ByRef connection As iormConnection = Nothing) As Object Implements iormRelationalDatabaseDriver.GetColumn
            ' TODO: Implement this method
            Throw New NotImplementedException()
        End Function
        ...
        ''' <returns></returns>
        Public Overrides Function GetForeignKeys(nativeTable As Object, foreignkeydefinition As ForeignKeyDefinition, Optional createOrAlter As Boolean = False, Optional ByRef connection As iormConnection = Nothing) As IEnumerable(Of Object) Implements iormRelationalDatabaseDriver.GetForeignKeys
            ' TODO: Implement this method
            Throw New NotImplementedException()
        End Function
        ''' <summary>
        ...
        Public Overrides Function DropDBParameterTable(Optional setupid As String = Nothing, Optional ByRef nativeConnection As Object = Nothing) As Boolean Implements iormRelationalDatabaseDriver.DropDBParameterContainer
            ...
            ''' drop table
            If droptable Then
                CurrentDBDriver.DropContainerObject(id:=DBParameterTablename)
                If Not CurrentDBDriver.HasContainerID(DBParameterTablename) Then
                    CoreMessageHandler(message:="OTDB PARAMETER TABLE  " & CurrentDBDriver.DBParameterContainerName & " DROPPED", containerID:=CurrentDBDriver.DBParameterContainerName, _
    ...
    Public MustInherit Class adonetViewReader
                ...
                Try
                    ...
                    If Me.HasProperty(ConstTPNCacheProperty) AndAlso Me.IsCacheInitialized Then
                        ...
                        Else
                            ...
                            '**
                            Dim aNewRecord As New ormRecord(tableID:=Me.ViewID, dbdriver:=Me.Connection.DatabaseDriver, runtimeOnly:=False)
            ...
            SyncLock aSqlSelectCommand.Connection
                ...
                Try
                    ...
                    If Me.HasProperty(ConstTPNCacheProperty) AndAlso Me.IsCacheInitialized Then
                        ...
                        Else
                            For Each row In dataRows
                                aNewRecord = New ormRecord(tableID:=Me.ViewID, dbdriver:=Me.Connection.DatabaseDriver, runtimeOnly:=False)
                                If aNewRecord.LoadFrom(row) Then
            ...
            Try
                ...
                If Me.HasProperty(ConstTPNCacheProperty) AndAlso Me.IsCacheInitialized Then
                    ...
                    Else
                        For Each row In dataRows
                            ''' infuse the records
                            aNewRecord = New ormRecord(tableID:=Me.ViewID, dbdriver:=Me.Connection.DatabaseDriver)
                            If aNewRecord.LoadFrom(row) Then
                                aCollection.Add(item:=aNewRecord)
                            Else
                ...
                SyncLock aCommand
                    ...
                    If Me.HasProperty(ConstTPNCacheProperty) AndAlso Me.IsCacheInitialized Then
                        ...
                        Else
                            For Each row In dataRows
                                ''' infuse the records
                                Dim aNewRecord As New ormRecord(tableID:=Me.ViewID, dbdriver:=Me.Connection.DatabaseDriver)
                                If aNewRecord.LoadFrom(row) Then
                                    aCollection.Add(item:=aNewRecord)
                                Else
        ...
        Implements iormRelationalTableStore.InfuseRecord
            ...
            Try
                ...
                If record Is Nothing OrElse CreateNewrecord Then record = New ormRecord(tableID:=Me.ViewID, dbdriver:=Me.Connection.DatabaseDriver)
    ...
    Public MustInherit Class adonetTableStore
        ...
        Implements iormRelationalTableStore.InfuseRecord
            ...
            Try
                ...
                '**
                ''' if record is not supplied take a bound record
                If record Is Nothing OrElse CreateNewrecord Then record = New ormRecord(tableID:=Me.TableID, dbdriver:=Me.Connection.DatabaseDriver)
                record.IsLoaded = True ' definitely loaded ! not created
